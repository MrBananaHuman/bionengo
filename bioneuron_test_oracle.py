...
def main():
	...
	if P['platform'] == 'workstation':
		...
		h = nengo.Lowpass(P['kernel']['tau'])  # synapse used to filter the training signals
		solver = nengo.solvers.LstsqL2(reg=0.01)
		# decoders_bio, info = solver(
		# 	h.filt(sim.data[probe_bio_spikes], dt=P['dt_nengo']),
		# 	h.filt(sim.data[probe_pre], dt=P['dt_nengo']))
		# 	# h.filt(h.filt(sim.data[probe_pre], dt=P['dt_nengo']), dt=P['dt_nengo']))
		...
		xhat_bio=np.sum(h.filt(sim.data[probe_bio_spikes], dt=P['dt_nengo'])*decoders_bio.ravel(),axis=1)

		rmse_bio=np.sqrt(np.average((sim.data[probe_dir]-xhat_bio)**2))
