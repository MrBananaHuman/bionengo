		...
		for inpt in ens_post.neuron_type.father_op.inputs.iterkeys():
			...
			for pre in range(ens_pre_info['pre_neurons']):
				...
				for syn in range(P['n_syn']):
					#todo: np.random version for nonoptimized locations
...
class Bahl():
	...
	def event_step(self,P,time,signals_spikes_pres):
		for inpt in signals_spikes_pres.iterkeys(): #for each input connection
			for pre in range(signals_spikes_pres[inpt]['spikes_in'].shape[1]): #for each input neuron
				if signals_spikes_pres[inpt]['spikes_in'][time][pre] > 0: #if neuron spikes at time
					for syn in self.synapses[inpt][pre]: #for each synapse in this conneciton
...
def run_biopop_events(P,ens_post,biopop,spikes_ideal,signals_spikes_pres):
	...
	for time in range(spikes_ideal.shape[0]):
		print 't=%.4f'%(time*P['dt_nengo'])
		for bioneuron in biopop.itervalues():
			bioneuron.event_step(P,time,signals_spikes_pres)
...
def plot_spikes_rates(P,best_result_file,loss):
	...
	plt.close()
	plots_per_subfig=1
	n_subfigs=int(np.ceil(rates_bio.shape[1]/plots_per_subfig))
	n_columns = 1
	...
	for row in range(n_rows):
		for column in range(n_columns):
			...
			for b in range(plots_per_subfig):
				if k>=rates_bio.shape[1]: break
				bio_rates_plot=ax.plot(timesteps,rates_bio[:,k][:len(timesteps)],linestyle='-')
				ideal_rates_plot=ax.plot(timesteps,rates_ideal[:,k][:len(timesteps)],linestyle='--',
					color=bio_rates_plot[0].get_color())
				ax.plot(0,0,color='k',linestyle='-',label='bioneuron')
				ax.plot(0,0,color='k',linestyle='--',label='LIF')
				bio_rates.append(rates_bio[:,k])
				ideal_rates.append(rates_ideal[:,k])
				k+=1
			...
			figure.savefig('bio_vs_ideal_rates_neurons_%s-%s'%(k-plots_per_subfig,k))
			plt.close(figure)
